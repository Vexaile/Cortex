...
- **4b** New **right-edge rail** hosting Cortex Agent, Notifications, Datasheets [audit MEDIUM] (Notifications only once Phase 8 exists).
- **4c** Dockable tool-window model: let bottom/right host any tool window; allow two visible at once; persist location [audit MEDIUM: single mutually-exclusive column].
